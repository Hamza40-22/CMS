...
class _HvacDetailScreenState extends State<HvacDetailScreen> {
  ...
  Timestamp? _acceptedAt;
  Timestamp? _completedAt;
  String _hvacStatus = "Work in Progress"; // Default Hvac status
  ...
  Future<void> _fetchComplaintData() async {
    try {
      ...
      if (complaintDoc.exists) {
        ...
        List<dynamic> HvacAcceptedDetails = acceptedDetails
            .where((detail) => detail['acceptedRole'] == 'hvac')
            .toList();

        setState(() {
          _isAccepted = HvacAcceptedDetails.isNotEmpty;
          _isCompleted = completedDetails.isNotEmpty;
          _hvacStatus = data['hvacStatus'] ?? 'Work in Progress'; // Fetch hvac status

          if (HvacAcceptedDetails.isNotEmpty) {
            var lastAcceptedDetail = HvacAcceptedDetails.last;
          ...
          if (completedDetails.isNotEmpty) {
            ...
            if (lastCompletedDetail['completedRole'] == 'hvac') {
              _isCompleted = true;
            } else {
              _isCompleted = false;
            }
  ...
  Future<void> _updateHvacStatus(String newStatus) async {
    try {
      bool confirm = await _showConfirmationDialog(
        'Update Hvac Status',
        'Are you sure you want to update the Hvac status to $newStatus?',
      );
      ...
      if (confirm) {
        ...
        if (newStatus == 'Completed') {
          ...
          await FirebaseFirestore.instance
              .collection('complaints')
              .doc(widget.complaintId)
              .update({'hvacStatus': newStatus});

          setState(() {
            _hvacStatus = newStatus;
          });

          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('hvac status updated to $newStatus')),
          );
        }
      }
    } catch (e) {
      print('Error updating hvac status: $e');
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Failed to update hvac status')),
      );
    }
  }
        ...
        child: Column(
          ...
          children: [
            ...
            if (_isAccepted) ...[
              ...
              // Display the Complete button only if the complaint is not marked as completed by a hvac role
              if (_hvacStatus != 'Completed' && (!_isCompleted || _completedBy != 'hvac')) ...[
                DropdownButton<String>(
                  value: _hvacStatus,
                  onChanged: (String? newValue) {
                    if (newValue != null) {
                      _updateHvacStatus(newValue);
                    }
                  },
                  items: <String>[
                    'Work in Progress',
              ...
              if (_isCompleted || _hvacStatus == 'Completed') ...[
                _buildGlassContainer(
